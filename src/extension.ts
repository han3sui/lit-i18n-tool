...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerTextEditorCommand("lit-i18n-tool.replaceSingle", async () => {
      ...
      if (list?.length) {
        ...
        if (picked) {
          return replaceSelectedContent(picked.description, "");
        }
      }
   ...
   */
  context.subscriptions.push(
    vscode.commands.registerTextEditorCommand("lit-i18n-tool.wrappdKey2", async () => {
      doWrape("replace2");
    })
  );
  context.subscriptions.push(
  ...
  context.subscriptions.push(
    vscode.commands.registerTextEditorCommand("lit-i18n-tool.wrappdKey1", async () => {
      doWrape("replace1");
    })
  );
   ...
   */
  context.subscriptions.push(
    vscode.commands.registerTextEditorCommand("lit-i18n-tool.updateKey", async () => {
      const activeEditor = window.activeTextEditor;
      if (!activeEditor) {
        return;
      }
      ...
      for (let i = 0; i < searchList.length; i++) {
        ...
        for (let j = 0; j < list.length; j++) {
          const obj: vscode.QuickPickItem = {
   ...
   */
  context.subscriptions.push(
    vscode.commands.registerCommand("lit-i18n-tool.check", async () => {
